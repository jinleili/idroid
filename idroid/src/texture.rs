...
pub fn from_path(
    ...
    encoder.copy_buffer_to_texture(
        wgpu::BufferCopyView {
            buffer: &temp_buf,
            offset: 0,
            row_pitch: channel_count * texture_extent.width,
...
pub fn update_by_path(
    ...
    encoder.copy_buffer_to_texture(
        wgpu::BufferCopyView {
            buffer: &temp_buf,
            offset: 0,
            row_pitch: if is_gray_pic { 1 } else { 4 } * texture_extent.width,
            image_height: texture_extent.height,
        },
        wgpu::TextureCopyView {
            texture: texture,
...
fn load_from_path(path: PathBuf, is_gray_pic: bool) -> (Vec<u8>, wgpu::Extent3d) {
    ...
    let img_raw = if is_gray_pic {
        img_load.to_luma().into_raw()
    } else {
        img_load.to_rgba().into_raw()
    };
